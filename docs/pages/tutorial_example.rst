...
If you want to run CONGA so that it clusters according to the exact peptide, and not the "stem" peptide, and thus allowing the discovery of peptides that differ by the placement of the variable modification, you can turn this feature off by setting ``--account_mods F``. Note that the assumptions of theoretical FDR control do not strictly hold in this case, because target peptides with variable modifications may be scored highly simply because they share similar b- and y-ions in common with the same target peptide but with possibly a different location of the same variable modifications. In any case, if you wish to run without clustering to the "stem" form, you will need to provide the target-decoy peptide pairs produced by tide-index.
